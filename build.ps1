...
<#
.Synopsis
   This script is used to deploy a fresh install of purple.lab
...
$LAB_HOSTS = ('PurpleLab-Splunk', 'PurpleLab-DC', 'PurpleLab-Win10')
...
function preflight_checks {
  ...
  if (-Not ($PackerOnly)) {
    ...
    if ($DrivesList.Count -gt 0) {
      Write-Host "The following drives have less than 80GB of free space. They should not be used for deploying purple.lab" -ForegroundColor yellow
      forEach ($drive in $DrivesList) {
        Write-Host "[*] $($drive.Name)"
      }
      Write-Host "You can safely ignore this warning if you are deploying purple.lab to a different drive." -ForegroundColor yellow
    }
...
function packer_build_box {
  ...
  if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "To file an issue, please visit https://github.com/neokjames/PurpleLab/issues/"
    break
  }
  Set-Location $CurrentDir
}
...
function move_boxes {
  ...
  if (-Not (Test-Path "$DL_DIR\Boxes\windows_2019_$PackerProvider.box")) {
    Write-Error "Windows 2019 box is missing from the Boxes directory. Quitting." -ForegroundColor red
    break
  }
...
function post_build_checks {
  ...
  $SPLUNK_CHECK = download -URL 'https://10.1.1.2:8000/en-US/account/login?return_to=%2Fen-US%2F' -PatternToMatch 'This browser is not supported by Splunk'
...
# Build Packer Boxes
if (!($VagrantOnly)) {
  packer_build_box -Box 'windows_2019'
